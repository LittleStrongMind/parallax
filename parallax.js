...
_ = {
	...
	random (min, max, previous, spread) {
		...
		if (!isNaN(previous) && Math.abs(value - previous) >= 25) {
...
class Parallax {
	...
	constructor (containerSelector, colors, layers, mergeLayers) {
		...
		if (!this.container) {
			console.error(`No container found with the selector '${selector}'.`);
			return false;
		}
